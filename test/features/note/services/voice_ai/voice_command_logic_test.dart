...
void main() {
  group('Voice Command Logic Tests', () {
    ...
    test('1. Global Reset - "Clear all formatting"', () {
      ...
      final styles = controller.document.collectStyle(
        0,
      ...
      expect(
        styles.attributes,
        isEmpty,
        reason: 'Global reset should wipe all attributes',
      );
    });
    ...
    test('2. Selection Interceptor - "Make this bold"', () {
      ...
      expect(
        controller.document.collectStyle(7, 1).attributes['bold'],
        isNotNull,
      );
    ...
    test('3. Global Document Lock - "Make everything red"', () {
      ...
      for (int i = 0; i < 15; i++) {
        expect(
          controller.document.collectStyle(i, 1).attributes['color']?.value,
          '#FF0000',
        );
      }
    });
    ...
    test('4. Line & Sentence Segmentation - "Make the first line bold"', () {
      ...
      expect(
        controller.document.collectStyle(0, 5).attributes['bold'],
        isNotNull,
      );
      expect(
        controller.document.collectStyle(12, 5).attributes['bold'],
        isNull,
      );
    });
    ...
    test('5. Paragraph Segmentation - "Make the first paragraph italic"', () {
      ...
      expect(
        controller.document.collectStyle(0, 11).attributes['italic'],
        isNotNull,
      );
      expect(
        controller.document.collectStyle(14, 11).attributes['italic'],
        isNull,
      );
    });
    ...
    test('6. Literal Phrase Targeting - "Underline golden retriever"', () {
      ...
      expect(
        controller.document.collectStyle(9, 16).attributes['underline'],
        isNotNull,
      );
      expect(
        controller.document.collectStyle(0, 5).attributes['underline'],
        isNull,
      );
    });
    ...
    test('7. Nth Occurrence - "Make the second instance of dog bold"', () {
      ...
      expect(
        controller.document.collectStyle(10, 3).attributes['bold'],
        isNull,
      );
      expect(
        controller.document.collectStyle(29, 3).attributes['bold'],
        isNotNull,
      );
    });
    ...
    test('8. Surgical List Engine - "Make menu items a checklist"', () {
      ...
      expect(
        controller.document.collectStyle(0, 10).attributes['list'],
        isNull,
      );
      expect(
        controller.document.collectStyle(12, 5).attributes['list']?.value,
        'unchecked',
      );
    });
    ...
    test('9. Relative Sizing - "Make the word warning look massive"', () {
      ...
      expect(
        controller.document.collectStyle(10, 7).attributes['size']?.value,
        21.0,
      );
    });
